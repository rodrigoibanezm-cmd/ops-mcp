...
import { getRedisKey } from '../tools/upstash/redis.js';
...
export async function callTool(name, args = {}) {
  if (WRITE_TOOLS.includes(name)) validateWriteToken(args.write_token);

  if (name === 'health.check') {
    return okResponse({ ok: true, service: 'ops-mcp', transport: 'http', status: 'alive' });
  }

  if (name === 'vercel.projects.list') return okResponse(await listProjects());
  ...
  if (name === 'upstash.redis.get') {
    return okResponse(await getRedisKey(args.key));
  }

  if (name === 'vercel.env.set') return okResponse(await setEnvVar(args));
  if (name === 'vercel.env.update') return okResponse(await updateEnvVar(args));
...
function validateWriteToken(writeToken) {
  if (!process.env.OPS_WRITE_TOKEN) throw new Error('missing_OPS_WRITE_TOKEN');
  if (!writeToken || writeToken !== process.env.OPS_WRITE_TOKEN) throw new Error('invalid_write_token');
}
