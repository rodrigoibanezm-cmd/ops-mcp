...
export const tools = [
  ...
  },
  {
    ...
    description: 'Dispara un nuevo deployment en Vercel para un proyecto.',
    inputSchema: {
      type: 'object',
      properties: {
        project_key: { type: 'string' }
...
export async function callTool(name, args = {}) {
  ...
  if (name === 'vercel.deploy.latest') {
    ...
    return {
      content: [{ type: 'text', text: JSON.stringify(await getLatestDeployment(args.project_key), null, 2) }]
    };
  }

  if (name === 'vercel.deploy.trigger') {
    if (!args.project_key) throw new Error('missing_project_key');

    return {
      content: [{ type: 'text', text: JSON.stringify(await triggerDeploy(args.project_key), null, 2) }]
    };
  }
  ...
  if (name === 'vercel.env.list') {
    ...
    return {
      content: [{ type: 'text', text: JSON.stringify(await listEnvVars(args.project_key), null, 2) }]
    };
  }

  if (name === 'vercel.env.set') {
    return {
      content: [{ type: 'text', text: JSON.stringify(await setEnvVar(args), null, 2) }]
    };
  }
  ...
  if (name === 'vercel.deploy.errors' || name === 'vercel.logs.errors') {
    ...
    return {
      content: [{ type: 'text', text: JSON.stringify(await getErrorDeployments(args.project_key, args.limit), null, 2) }]
    };
  }
  ...
  if (name === 'vercel.deploy.inspect') {
    ...
    return {
      content: [{ type: 'text', text: JSON.stringify(await inspectDeployment(args.deployment_uid, args.project_key), null, 2) }]
    };
  }
